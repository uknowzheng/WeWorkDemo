      ...
      },
      {
        ...
        messages: [
          ...
          },
          {
            ...
            type: 1,
            content:
              "给兄弟们发点好康的@::tt;;@17c07d6a3cd57c6654c75728987b93dc@",
            date: new Date(1637131487000),
            showTime: false,
          },
          {
...
const getters = {
  getTotalNewMsgs(state) {
    ...
    for (let i = 0; i < state.chatlist.length; i++) {
      if (
        !state.chatlist[i].info.notDisturb &&
        state.chatlist[i].newMsgNum > 0
      ) {
  ...
  searchedChatlist(state, getters, rootState) {
    let remarkSessions = state.chatlist.filter(
      (sessions) =>
        sessions.info.remark != null &&
        sessions.info.remark.includes(rootState.system.searchText)
    );
    let sessions = state.chatlist.filter((sessions) =>
      sessions.info.nickname.includes(rootState.system.searchText)
    );
    let add = sessions.filter(
      (x) => !remarkSessions.find((y) => x.chatId === y.chatId)
    );
